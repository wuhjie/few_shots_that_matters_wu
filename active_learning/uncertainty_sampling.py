...
def search_in_trn(index_list, loaders):
    ...
    new_loader = []
    for i in range(len(loaders.raw_dataset.trn_egs)):
        for j in range(len(loaders.raw_dataset.trn_egs[i])):
            print("j: ", j)
            print("index: ", loaders.raw_dataset.trn_egs[i][j][index_list[0]])
